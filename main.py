...
class LSTM(NNModel):
    ...
    def predict_proba(self, x, params):
        ...
        even, _, _ = Rnn.lstm_cell(even,
                                   jnp.zeros((x.shape[1], self.config['lstm:1even']['hidden_dim'])),
                                   jnp.zeros((x.shape[1], self.config['lstm:1even']['hidden_dim'])),
                                   params['lstm:1even']['Ws'], params['lstm:1even']['Us'], params['lstm:1even']['Bs'])

        odd, _, _ = Rnn.lstm_cell(odd,
                                  jnp.zeros((x.shape[1], self.config['lstm:1odd']['hidden_dim'])),
                                  jnp.zeros((x.shape[1], self.config['lstm:1odd']['hidden_dim'])),
                                  params['lstm:1odd']['Ws'], params['lstm:1odd']['Us'], params['lstm:1odd']['Bs'])
